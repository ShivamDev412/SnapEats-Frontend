      ...
      <div className="mb-4">
        ...
        >
          <select
            ...
            {...register(id)}
          >
            <option value="" disabled selected className="bg-white text-zinc-950">
              {placeholder}
            </option>
            {data.map((option, index) => (
              <option key={index} value={option.value} className="bg-white text-zinc-950">
                {option.label}
              </option>
            ))}
          </select>
          <div className="absolute right-3 pointer-events-none">
            <svg className="w-5 h-5 text-zinc-700" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
