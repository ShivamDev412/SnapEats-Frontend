...
const CartItem: React.FC<CartItemProps> = ({ cartItem }) => {
  ...
  return (
    <div
      className="my-4 p-4 bg-zinc-800 rounded-lg shadow-md"
      key={cartItem.id}
    >
      <div className="flex justify-between items-center mb-2">
        <p className="text-lg sm:text-base font-semibold text-zinc-100">{cartItem.name}</p>
        <p className="text-lg sm:text-base font-semibold text-zinc-100">
          ${cartItem.price.toFixed(2)}
        </p>
      </div>
      <div className="text-sm sm:text-xs text-zinc-400 mb-2">
      ...
      <textarea
        ...
        className="w-full p-2 mb-2 bg-zinc-600 rounded-md text-zinc-100 placeholder-zinc-400 resize-none focus:outline-none focus:ring-2 focus:ring-zinc-400 text-base sm:text-sm"
      ></textarea>
      <div className="flex justify-between items-center mt-2 text-sm sm:text-xs">
        <QuantitySection quantity={cartItem.quantity} />
        <p className="font-medium text-lg sm:text-base text-zinc-100">
